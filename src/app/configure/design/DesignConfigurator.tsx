         ...
         <div className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
            ...
            >
               <div className="relative w-full h-full ">
                  <NextImage src={imageUrl} alt="user-image" fill className="pointer-events-none" />
               </div>
            </Rnd>
         </div>
                           ...
                           <div className="mt-3 flex items-center space-x-3">
                              {COLORS.map((color) => (
                                 <Radio
                                    ...
                                    className={({ checked }) =>
                                       cn(
                                          "relative -m-0.5 flex cursor-pointer items-center jusitfy-center rounded-full p-0.5 active:ring-0 focus:ring-0 active:outline-none focus:outline-none border-2 border-transparent",
                                          {
                           ...
                           <DropdownMenu>
                              ...
                              <DropdownMenuContent>
                                 {MODELS.options.map((model) => (
                                    ...
                                    >
                                       <Check className={(cn("mr-2 h-4 w-4"), model.label === options.model.label ? "opactiy-100" : "opacity-0")} />
                                       {model.label}
                                    </DropdownMenuItem>
                                 ))}
                              </DropdownMenuContent>
                                    ...
                                    >
                                       <span className="flex items-center">
                                          <span className="flex flex-col text-sm">
                                             ...
                                             {option.description ? (
                                                <Description as="span" className="text-gray-500 text-sm mt-1">
               ...
               <div className="w-full h-full flex justify-end items-center">
                  <div className="w-full flex gap-6 items-center">
                     <p className="font-medium whitespace-nowrap">
                        {formatPrice((BASE_PRICE + options.finish.price + options.materials.price) / 100)}
                     </p>
