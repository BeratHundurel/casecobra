   ...
   return (
      <ol className="rouded-md bg-white lg:flex lg:rounded-none lg:border-1 lg:border-r lg:border-gray-200">
         {STEPS.map((step, i) => {
            ...
            const isCompeted = STEPS.slice(i + 1).some((step) => pathname.endsWith(step.url));
            const imgPath = `/snake-${i + 1}.png`;
            return (
               <li key={step.name} className="relative overflow-hidden lg:flex-1">
                  <div>
                     <span
                        className={cn("absolute left-0 top-0 h-full w-1 bg-zinc-400 lg:top-auto lg:h-1 lg:w-full", {
                           "bg-zinc-700": isCurrent,
                           "bg-primary": isCompeted,
                        })}
                        aria-hidden="true"
                     />
